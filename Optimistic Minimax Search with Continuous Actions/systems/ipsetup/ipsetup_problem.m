...
maxx = [pi; 15*pi]; maxu = 3; % (not enough to swing upright in one go) original = 3
maxw_percentage = 0.05; % maximum disturbance as percentage of maxu
maxw = maxw_percentage * maxu; % maximum distrubance
...
switch what
    ...
    case 'model'
        ...
        PAR.maxu = maxu;
        PAR.maxw = maxw;
        PAR.maxw_percentage = maxw_percentage;
        % stochasticity
...
%         PAR.J = 3.24e-5;            % (rotor and disk) for CD
        ...
        model.det = isempty(model.stoch_prob);
        if ~model.det, model = stochact_mdpwrapper(model, 'prepare'); end;
